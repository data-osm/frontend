...
*/
   ...
   */
  initialiseFormsLayers(loadTOCLayers: boolean = false, map:Map) {
    this.formsLayers = this.fb.group({
      layers: this.fb.array([this.createInputFormsLayer()])
    })
    new CartoHelper(map).map.getLayers().on('propertychange', (ObjectEvent) => {
      this.addAllLayersInTOC(map)
    })
  }
   ...
   */
  addAllLayersInTOC(map:Map) {
    this.formsLayersArray = this.formsLayers.get('layers') as FormArray;
    if (this.getAllLayersInTOC(map).length > 0) {
    ...
    for (let index = 0; index < this.getAllLayersInTOC(map).length; index++) {
      const element = this.getAllLayersInTOC(map)[index];
   ...
   * get Layers in TOC
   */
  getAllLayersInTOC(map:Map): searchLayerToDownlodModelInterface[] {
