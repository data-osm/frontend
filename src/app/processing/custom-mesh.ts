...
export class CustomInstancedBufferGeometry extends InstancedBufferGeometry {
    ...
    computeInstanceBoundingSphere() {
        ...
        if (this.instanceBoundingSphere === null) {

            this.instanceBoundingSphere = new Sphere();

        }

        const aInstancePosition = this.attributes.aInstancePosition;
        const morphAttributesPosition = this.morphAttributes.position;

        // @ts-expect-error
        if (aInstancePosition && aInstancePosition.isGLBufferAttribute) {

            console.error('THREE.BufferGeometry.computeBoundingSphere(): GLBufferAttribute requires a manual bounding sphere.', this);

            this.instanceBoundingSphere.set(new Vector3(), Infinity);

            return;

        }

        if (aInstancePosition) {

            // first, find the center of the bounding sphere

            const center = this.instanceBoundingSphere.center;

            // @ts-expect-error
            _box.setFromBufferAttribute(aInstancePosition);
            ...
            for (let i = 0, il = aInstancePosition.count; i < il; i++) {

                _vector.fromBufferAttribute(aInstancePosition, i);
                ...
                for (let i = 0, il = morphAttributesPosition.length; i < il; i++) {
                    ...
                    for (let j = 0, jl = morphAttribute.count; j < jl; j++) {
                        ...
                        if (morphTargetsRelative) {
                            ...
                            _offset.fromBufferAttribute(aInstancePosition, j);
                            _vector.add(_offset);
...
export class PointMesh extends Mesh<CustomInstancedBufferGeometry, ShaderMaterial, Object3DEventMap> {
    ...
    raycast(raycaster: Raycaster, intersects) {
        ...
        const raycastTimes = this.geometry.instanceCount;
...
function checkIntersection(object: Mesh, material, raycaster, ray, pA, pB, pC, point) {
...
function checkGeometryIntersection(object: Mesh, material, raycaster, ray, uv, uv1, normal, a, b, c) {
