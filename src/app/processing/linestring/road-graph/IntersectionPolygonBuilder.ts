...
import { getIntersectionLineLine } from "../../building/roof/utils";
...
export class Segment {
	...
	public updateIntersectionsAlphaForSide(intersections: LineIntersection[], segment: [Vec2, Vec2]): void {
		for (const intersection of intersections) {
			intersection.alpha = getPointProgressAlongLineSegment(
				segment[0],
				segment[1],
				intersection.point
			);
	...
	public getTrimmedEnd(): Vec2 {
		...
		const progress = getPointProgressAlongLineSegment(this.start, this.end, this.trimTo.point, true);
...
export default class IntersectionPolygonBuilder {
	...
	public getPolygon(): Vec2[] {
		...
		for (let i = 0; i < this.segments.length; i++) {
			...
			const intersection: [number, number] = getIntersectionLineLine(
...
function getPointProgressAlongLineSegment(start: Vec2, end: Vec2, point: Vec2, clamp: boolean = true): number {
