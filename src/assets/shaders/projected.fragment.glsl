...
void main() {
	...
	vec4 baseColor = texture(tMapp, vec3(vUv, layer));
	...
	vec3 mask = texture(tMapp, vec3(vUv, layer + 2)).xyz;
    ...
    // if(vTextureId != 0) {
	vec3 normalMap = getNormalValue(layer + 1, tMapp, normal, vViewPosition, vUv, 3.0);
	normal = normalMap;
// }
