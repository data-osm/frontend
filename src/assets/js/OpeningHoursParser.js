 ...
 * Model JS classes
 */
var extend = function (parameter) {

	return Object.assign(Object.create(Object.getPrototypeOf(parameter)), parameter)
	...
	Day.prototype.copyIntervals = function(intervals) {
		...
		for(var i=0; i < intervals.length; i++) {
			if(intervals[i] != undefined && intervals[i].getStartDay() == 0 && intervals[i].getEndDay() == 0) {
				this._intervals.push(extend(intervals[i]));
			}
		}
	...
	Week.prototype.copyIntervals = function(intervals) {
		...
		for(var i=0; i < intervals.length; i++) {
			if(intervals[i] != undefined) {
				// let clone = Object.assign(Object.create(Object.getPrototypeOf(intervals[i])), intervals[i])

				// this._intervals.push(clone);
				this._intervals.push(extend(intervals[i]));
			}
		}
	};
...
//OTHER METHODS
	...
	OpeningHoursParser.prototype._tokenize = function(block) {
		...
		var position = result.findIndex((x)=>x=="")
		// var position = $.inArray((x)=>x=="", result);
		while( ~position ) {
